...
const HELP_TEXT = `Usage: node .squad/scripts/create-app.mjs --role <role> [--owner <github-username>] [--prefix <app-name-prefix>]
...
Options:
  ...
  --keys-dir <path>  Save PEMs in this directory (default: ~/.config/squad/{owner}/keys)
  ...
  --import-key <path>
                     Import a downloaded PEM into the configured keys directory
...
function parseArgs(argv) {
  ...
  const valueFlags = new Set(["--role", "--owner", "--prefix", "--keys-dir", "--import-key", "--name"]);
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (flag === "--keys-dir") values.keysDir = value;
...
function getKeysDir(owner, keysDirOverride) {
  const configuredPath = keysDirOverride ?? `~/.config/squad/${owner}/keys`;
  return resolve(expandHomePath(configuredPath));
}

function getConfiguredKeysDir(owner, keysDirOverride) {
  return keysDirOverride ?? `~/.config/squad/${owner}/keys`;
}
...
function ensureIdentityKeysDirConfigured(projectRoot, configuredKeysDir) {
  const config = loadIdentityConfig(projectRoot) ?? { tier: "per-role", apps: {} };
  if (typeof config.keysDir === "string" && config.keysDir.trim()) {
    return;
  }

  config.keysDir = configuredKeysDir;
  saveIdentityConfig(projectRoot, config);
}

function migrateLegacyRepoKey(projectRoot, role, keysDir) {
  const legacyKeyPath = join(projectRoot, ".squad", "identity", "keys", `${role}.pem`);
  const ownerDir = dirname(keysDir);
  const externalPemPath = join(keysDir, `${role}.pem`);

  if (!existsSync(legacyKeyPath)) {
    return;
  }

  const stat = lstatSync(legacyKeyPath);
  if (stat.isSymbolicLink()) {
    rmSync(legacyKeyPath, { force: true });
    return;
  }

  if (existsSync(externalPemPath)) {
    return;
  }

  ensurePrivateDir(ownerDir);
  ensurePrivateDir(keysDir);
  renameSync(legacyKeyPath, externalPemPath);
  chmodSync(externalPemPath, 0o600);
}
...
function saveCredentials(projectRoot, role, keysDir, appData) {
  const appsDir = join(projectRoot, ".squad", "identity", "apps");
  const ownerDir = dirname(keysDir);
  const externalPemPath = join(keysDir, `${role}.pem`);
  const oauthPath = join(keysDir, `${role}.oauth.json`);
  ...
  writeFileSync(externalPemPath, appData.pem, { encoding: "utf8", mode: 0o600 });
  chmodSync(externalPemPath, 0o600);
  writeFileSync(
    oauthPath,
    `${JSON.stringify(
  ...
  return { externalPemPath, oauthPath, appPath };
}
...
function importPrivateKey(importPathArg, role, keysDir) {
  ...
  const ownerDir = dirname(keysDir);
  const destinationPath = join(keysDir, `${role}.pem`);

  ensurePrivateDir(ownerDir);
  ensurePrivateDir(keysDir);
  writeFileSync(destinationPath, pemContent, { encoding: "utf8", mode: 0o600 });
...
async function main() {
  ...
  if ((args.generateKey || args.importKey) && !args.owner) {
    fail("--owner is required with --generate-key and --import-key.");
  }
  ...
  if (args.generateKey) {
    ...
    console.log(
      `Then run: node .squad/scripts/create-app.mjs --import-key ~/Downloads/${appSlug}*.pem --role ${args.role} --owner ${args.owner}`,
    );
    return;
  }

  const owner = args.owner ?? getDefaultOwner();
  const configuredKeysDir = getConfiguredKeysDir(owner, args.keysDir);
  const keysDir = getKeysDir(owner, args.keysDir);

  if (args.importKey) {
    ensureIdentityKeysDirConfigured(projectRoot, configuredKeysDir);
    const savedPath = importPrivateKey(args.importKey, args.role, keysDir);
    console.log(`Imported private key for role "${args.role}".`);
  ...
  migrateLegacyRepoKey(projectRoot, args.role, keysDir);
  ensureIdentityKeysDirConfigured(projectRoot, configuredKeysDir);
  ...
  const { externalPemPath, oauthPath, appPath } = saveCredentials(projectRoot, args.role, keysDir, appData);
  ...
  console.log(`Private key: ${externalPemPath}`);
  console.log(`OAuth client secret: ${oauthPath}`);
  console.log(`Registration: ${appPath}`);
  console.log(`Configured keysDir: ${configuredKeysDir}`);
  if (args.icon) {
