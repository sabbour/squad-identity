...
function printCommandHelp(command) {
  const help = {
    ...
    setup: `Usage: squad-identity setup [target-repo] [--force]\n\nThe primary entry point for squad-identity setup. Runs the full guided flow\nidempotently — skips already-configured steps unless --force is passed.\n\nPhases:\n  1. Init — install extension, skill, config template (if missing)\n  2. Discover — read .squad/team.md for agent roles needed\n  3. Apps — for each role missing an app: prompt [C]reate / [i]mport / [s]kip\n            (roles that already have app + PEM are skipped automatically)\n  4. Install — open browser for ALL uninstalled apps at once, wait for\n              confirmation, then verify installation IDs\n  5. Configure — update agent charters with ROLE_SLUG + copilot-instructions\n  6. Health — run doctor (only shows roles used by the current team)\n\nOptions:\n  --force    Re-prompt for all roles, even if already configured`,
